...
**Domain:** Household budgeting PWA (2-person shared, mobile-first)
**Researched:** 2026-04-21
**Confidence:** HIGH — all versions verified against npm registry as of today

---

## Critical Version Corrections

The plan was written targeting Next.js 15 and Recharts 2.x. Both are outdated as of April 2026.
...
**Recommendation: Use Next.js 16.2.4 (latest).**
...
**If you have a strong reason to stay on 15:** Pin `next@15.5.15` (the active LTS backport). No other packages require 16 specifically. Everything below is compatible with either.
...
pnpm create next-app@latest hearth-budget --typescript --tailwind --app --src-dir --import-alias "@/*"
...
npx shadcn@latest add button input form card dialog sheet tabs

# Dev
pnpm add -D supabase prettier
```
...
If any boilerplate or tutorials reference Zod v3 patterns, be aware of these breaking changes in v4:
...
For a greenfield project, just use v4 patterns from the start. `@hookform/resolvers@5.2.2` is verified compatible with Zod 4.

---

## PWA Configuration Sketch (Serwist)

The plan references `next-pwa`. Replace with `@serwist/next`:
...
The web app manifest goes in `src/app/manifest.ts` (Next.js App Router convention, not a static JSON file).

---

## Supabase Auth Pattern for Next.js App Router

`@supabase/ssr` requires three client factories:

```
src/lib/supabase/
  client.ts      — browser client (createBrowserClient)
  server.ts      — Server Component client (createServerClient + cookies())
  middleware.ts  — middleware client (createServerClient + request/response cookies)
```

Middleware must run on every protected route to refresh the session cookie. Use Next.js `middleware.ts` at the root with a matcher that includes `/app/*`.
...
---
*Stack research for: Hearth Budget (household budgeting PWA)*
*Researched: 2026-04-21*
